    ...
    return (
      <div className="min-h-screen bg-zinc-900 flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-2xl font-bold text-zinc-100 mb-3">Edit Already Used</h1>
          <p className="text-zinc-400 mb-6">You have already used your one-time edit. Each participant gets one change.</p>
          <Link href="/250" className="text-amber-400 hover:text-amber-300">← Back to KRML 250</Link>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-zinc-900 flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-2xl font-bold text-zinc-100 mb-3">Song Replaced!</h1>
          <p className="text-zinc-400 mb-6">Your submission has been updated.</p>
  ...
  return (
    <div className="min-h-screen bg-zinc-900 px-4 py-10">
      <div className="max-w-xl mx-auto">
        ...
        <h1 className="text-3xl font-bold text-zinc-100 mb-2">Edit Your Submission</h1>
        <p className="text-zinc-400 mb-2">You have <strong className="text-amber-400">one edit</strong> available. Replace one of your three songs.</p>
        ...
        {submissions.length > 0 && (
          <div className="mb-6">
            <h2 className="text-sm font-semibold text-zinc-400 mb-3 uppercase tracking-wider">Your current songs</h2>
            <div className="space-y-2">
              {submissions.map((sub) => (
                <div key={sub.id} className="flex items-center gap-3 bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-3">
                  <span className="text-amber-400 font-bold w-6">#{sub.submission_slot}</span>
                  <div>
                    <div className="text-zinc-100 text-sm font-medium">{sub.song?.canonical_title ?? sub.song_id}</div>
                    <div className="text-zinc-500 text-xs">{sub.song?.canonical_artist}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <select value={replaceSlot} onChange={(e) => setReplaceSlot(Number(e.target.value))} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-3 text-zinc-100 focus:outline-none focus:border-amber-500">
          ...
          <div>
            ...
            <textarea required value={whyText} onChange={(e) => setWhyText(e.target.value)} rows={3} maxLength={500} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-3 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-amber-500 resize-none" placeholder="Tell us why..." />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <select value={townTag} onChange={(e) => setTownTag(e.target.value)} className="bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-zinc-100 text-sm focus:outline-none focus:border-amber-500">
              <option value="">Any town</option>
              {TOWNS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
            </select>
            <select value={decadeTag} onChange={(e) => setDecadeTag(e.target.value)} className="bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-zinc-100 text-sm focus:outline-none focus:border-amber-500">
          ...
          <button type="submit" disabled={submitting} className="w-full bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-zinc-900 font-bold py-4 rounded-xl text-lg transition-colors">

    ...
    return (
      <div className="min-h-screen bg-[#F5F3EF] flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <h1 className="font-serif text-2xl font-bold text-[#1F1F1F] mb-3">Prediction Locked In!</h1>
          <p className="text-[#6B6560] mb-2">Your Top 5 prediction is saved. You can update it any time before July 3, 2026.</p>
          <div className="bg-[#C9A66B]/15 border border-[#C9A66B]/40 rounded-xl p-4 mb-6 text-[#B8924A] text-sm">
            🎁 If your Top 5 matches the official list, you&apos;ll win a limited-edition KRML 250 hoodie!
          </div>
          <Link href="/250" className="bg-[#2F5D62] hover:bg-[#245059] text-white font-bold px-6 py-3 rounded-xl transition-colors">Back to Home</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] px-4 py-10">
      <div className="max-w-xl mx-auto">
        ...
        <h1 className="font-serif text-3xl font-bold text-[#1F1F1F] mb-2">Predict the Top 5</h1>
        <p className="text-[#6B6560] mb-2">{existing ? "Update your prediction" : "Guess the five songs that will top the KRML 250 list"}. Match all 5 in order and win a hoodie!</p>
        <div className="bg-[#C9A66B]/15 border border-[#C9A66B]/40 rounded-xl px-4 py-3 text-[#B8924A] text-sm mb-8">
          🎁 Correct predictions revealed July 3, 2026. You can update until then.
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          {picks.map((pick, index) => (
            <div key={index} className="flex items-start gap-3">
              <div className="w-8 h-8 rounded-full bg-[#2F5D62]/10 border border-[#2F5D62]/30 flex items-center justify-center text-[#2F5D62] font-bold text-sm flex-shrink-0 mt-9">{index + 1}</div>
          ...
          <button type="submit" disabled={submitting}
            className="w-full bg-[#2F5D62] hover:bg-[#245059] disabled:opacity-50 text-white font-bold py-4 rounded-xl text-lg transition-colors mt-4">

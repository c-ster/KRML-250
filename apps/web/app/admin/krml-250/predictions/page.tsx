...
export default function AdminPredictions() {
  ...
  useEffect(() => {
    adminApi.predictions().then(setPredictions).catch((e) => setError(e.detail || "Failed to load")).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="p-6">
      ...
      {loading ? <p className="text-zinc-400 animate-pulse">Loading...</p> : predictions.length === 0 ? <p className="text-zinc-500">No predictions submitted yet.</p> : (
        <div className="space-y-4">
          {predictions.map((pred) => (
            <div key={pred.id} className="bg-zinc-800 border border-zinc-700 rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-zinc-600 text-xs">{new Date(pred.created_at).toLocaleDateString()}{pred.locked_at && <span className="ml-2 text-amber-400">🔒 Locked</span>}</div>
              </div>
          ...
          <p className="text-zinc-600 text-sm">{predictions.length} total predictions</p>
        </div>
      )}
    </div>
  );

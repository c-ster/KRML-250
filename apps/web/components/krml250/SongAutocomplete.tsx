  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      <div className="relative">
        <input type="text" value={query} onChange={(e) => { setQuery(e.target.value); if (value) onChange(null); }} onFocus={() => results.length > 0 && setOpen(true)} placeholder={placeholder} disabled={disabled} className="w-full bg-[#F5F3EF] border border-[#D8D4CE] rounded-lg px-4 py-3 text-[#1F1F1F] placeholder-[#8A8480] focus:outline-none focus:border-[#2F5D62] disabled:opacity-50 disabled:cursor-not-allowed pr-10" />
      ...
      {open && !loading && query.length >= 2 && results.length === 0 && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-[#D8D4CE] rounded-lg px-4 py-3 text-[#6B6560] text-sm">No songs found for &ldquo;{query}&rdquo;. Try a different title or artist.</div>
      )}
    </div>
  );
}

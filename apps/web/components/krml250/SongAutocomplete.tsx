...
export function SongAutocomplete({ value, onChange, placeholder = "Search by title or artist...", disabled = false, label }: Props) {
  ...
  const clear = () => { onChange(null); setQuery(""); setResults([]); setOpen(false); setShowAddForm(false); };
  ...
  const noResults = !loading && !value && open && query.length >= 2 && results.length === 0;
    ...
    <div ref={containerRef} className="relative w-full">
      ...
      <div className="relative">
        <input
          type="text"
          value={query}
          onChange={(e) => { setQuery(e.target.value); if (value) onChange(null); setShowAddForm(false); }}
          onFocus={() => { if (results.length > 0) setOpen(true); }}
          placeholder={placeholder}
          disabled={disabled}
          className="w-full bg-[#F5F3EF] border border-[#D8D4CE] rounded-lg px-4 py-3 text-[#1F1F1F] placeholder-[#8A8480] focus:outline-none focus:border-[#2F5D62] disabled:opacity-50 disabled:cursor-not-allowed pr-10"
        />
      ...
      {value && (
        <div className="mt-1 text-xs text-[#2F5D62] font-medium">
          ✓ {value.canonical_title} — {value.canonical_artist}{value.decade && ` (${value.decade})`}
        </div>
      )}
      ...
      {noResults && !showAddForm && (
        <div className="mt-2 bg-[#F5F3EF] border border-[#D8D4CE] rounded-lg px-4 py-3">
          <p className="text-[#6B6560] text-sm mb-2">No songs found for &ldquo;{query}&rdquo;.</p>
          <button
            type="button"
            onClick={() => { setAddTitle(query); setAddArtist(""); setShowAddForm(true); setOpen(false); }}
            className="text-sm font-semibold text-[#2F5D62] hover:text-[#245059] underline"
          >
            + Don&apos;t see your song? Add it to the catalog
          </button>
        </div>
      )}
      ...
      {showAddForm && (
        <div className="mt-2 bg-white border border-[#C9A66B]/50 rounded-xl p-4 space-y-3">
          ...
          <input type="text" value={addTitle} onChange={(e) => setAddTitle(e.target.value)}
            ...
            className="w-full bg-[#F5F3EF] border border-[#D8D4CE] rounded-lg px-3 py-2 text-[#1F1F1F] placeholder-[#8A8480] text-sm focus:outline-none focus:border-[#2F5D62]" />
